...
const DevicesAndInfrastructure = () => {
    ...
    const handleCheckDevice = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.get(`http://localhost:5000/api/ota/check-device?device=${selectedDevice}`, {
    ...
    const handleUpdateViaLink = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post(`http://localhost:5000/api/ota/update-link/${selectedDevice}`, { url: gitLink }, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleUploadFirmware = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post(`http://localhost:5000/api/ota/upload/${selectedDevice}`, formData, {
